...
class InventoryAgent:
    ...
    def __init__(self, model_provider: OpenAIProvider):
        ...
        # Define the LLM-powered agent with tools for querying inventory and suppliers
        self.agent = Agent(
            model=OpenAIModel(OPENAI_DEFAULT_MODEL, provider=model_provider),
    ...
    def process_order_llm(self, order: Order) -> InventoryResult:
        """
        Process an order using the LLM-based approach.
        Converts the order into JSON, sends it to the agent, and returns the result.
        """
        try:
            # Convert order to JSON for agent consumption
            user_message = order.model_dump_json(indent=2)
            
            # Run synchronously through the LLM agent
    ...
    def process_order_direct(self, order: Order) -> InventoryResult:
        """
        Deterministic path: process orders using explicit rules and validations
        (no LLM involvement).
        """
        ...
        try:
            ...
            # Build full stock order
            stock_order = StockOrder(items=stock_orders)

            # Validate available cash to fund purchase
            self._check_against_cash_balance(stock_order, order_date)

            # Validate delivery timeline against customer expectation
        ...
        # Handle different types of fulfillment errors explicitly
            ...
            log(str(e), LogLevel.ERROR)
            # Attempt to enrich user message with earliest restock date
            extra = ""
            try:
        ...
        # Return failure result when any exception is raised
        return InventoryResult(
            is_success=False,
            stock_order=None,
    ...
    def _process_order_item(
        ...
        """
        Process a single order item by checking inventory levels and deciding whether 
        a stock order is required. Can raise custom exceptions if item cannot be fulfilled.
        """
        # Fetch static item data from catalog
        inventory_item = get_inventory_item(item.item_name)
        # Fetch live stock level at given order date
        stock_level_data = get_stock_level(item.item_name, order_date)

        if inventory_item is None or stock_level_data.empty:
            # Item not found in inventory → raise error with substitute suggestions
        ...
        # Case 1: insufficient stock to fulfill request
        if stock_level < item.quantity:
            ...
            # Accept order only if restock can arrive before expected delivery window
            if restock_delivery_date + timedelta(days=1) <= expected_order_delivery_date:
                ...
                # Otherwise, raise shortage error with earliest restock info
        ...
        # Case 2: stock sufficient, but leaves inventory below minimum threshold
        ...
        # Case 3: enough stock and no restock needed
        return None

    def _check_against_cash_balance(self, stock_order: StockOrder, as_of_date: datetime) -> bool:
        """
        Validate whether company has enough cash balance to pay for the stock order.
        """
    ...
    def _check_latest_delivery_date(self, expected_delivery_date: datetime, stock: StockOrder) -> bool:
        """
        Ensure that all ordered items can be delivered on or before the expected delivery date.
        """
    ...
    def _get_system_prompt(cls) -> str:
        """
        System prompt for the LLM agent that defines its role and responsibilities.
        """
        return """
...
def canonical_targets_for(query: str) -> Optional[List[str]]:
    """
    Given a user query string, return a list of canonical target product names
    that best match the query using alias maps and heuristics.
    """
    # Normalize/synonymize the query text (e.g., lowercase, replace aliases)
    q = _synonymize(query)

    # Step 1: Try to match against known alias patterns in ALIAS_MAP
    for pattern, targets in ALIAS_MAP:
        if re.search(pattern, q):
            return targets

    # Step 2: Apply fallback size-based heuristics if no alias matched

    # Detect explicit "24x36" → large poster paper
    if re.search(r"\b24x36\b", q):
        return ["Large poster paper (24x36 inches)", "Poster paper"]

    # Detect explicit "11x17" → poster or large poster paper
    if re.search(r"\b11x17\b", q):
        return ["Poster paper", "Large poster paper (24x36 inches)"]

    # Detect "8.5x11" or "letter size" → standard copy/letter/A4
    if re.search(r"\b8\.5x11\b|\bletter\s*size(d)?\b", q):
        return ["Standard copy paper", "Letter-sized paper", "A4 paper"]

    # Step 3: If nothing matches, return None
    return None
...
def run_test_scenarios(
    ...
    Prints responses, updates financials, and saves results to test_results.csv.
    """
    # Step 1: Initialize database
    log("Initializing Database...", LogLevel.INFO)
    init_database(db_engine=db_engine)

    # Step 2: Load and preprocess sample requests from CSV
    try:
        quote_requests_sample = pd.read_csv("quote_requests_sample.csv")
        # Parse request_date column into datetime objects
        ...
        # Fatal error if file not found or data invalid
    ...
    # Step 4: Create orchestration agent that handles requests
    ...
    # Step 5: Take initial financial snapshot at earliest date
    initial_date = quote_requests_sample["request_date"].min().strftime("%Y-%m-%d")
    report = generate_financial_report(initial_date)
    current_cash = report["cash_balance"]
    current_inventory = report["inventory_value"]

    # Step 6: Decide how many requests to process
    ...
    # Step 7: Process requests sequentially
    for idx, row in quote_requests_sample.iterrows():
        if idx >= sample_limit:
            break

        # Extract context for readability
        ...
        # Attach request date to the prompt for clarity
        request_with_date = f"{request_text} (Date of request: {request_date})"

        # Run orchestration agent on this request
        response = orchestrator.process_quote_request(request_text=request_with_date)

        # Update financials after this request
        report = generate_financial_report(request_date)
        current_cash = report["cash_balance"]
        current_inventory = report["inventory_value"]

        # Print response and updated status
    ...
    # Step 8: Generate final report after all requests
    ...
    # Return collected results as Python list
    return results
